...
The exponential distribution can be simulated in R with the `rexp(n, lambda)` function where lambda $\lambda$ represents the rate parameter. The mean of an exponential distribution is $\mu = \frac{1}{\lambda}$ and the standard deviation is $\sigma = \frac{\frac{1}{\lambda}}{\sqrt{n}}$. 

According the CLT if a sample consists of at least 30 independent observations and the data are not strongly skewed, then the distribution of the sample mean, the sampling distribution, is well approximated by a normal model which expressed in mathematical notation is: $\bar{x}_{n}$ ~ $N(\mu, {\sigma})$.
...
$\sigma^{2}$ in our case is `r round(var(exp_sample_means), 2)` which is close to the variance of the theoretical distribution we mentioned above.
...
ggplot(data, aes(x = exp_sample_means)) + 
  geom_histogram(binwidth = 0.4, color = 'black', fill = 'yellow', aes(y = ..density..)) +
...
The complete source code of this report can be found on [GitHub](https://github.com/amansinha16/Statistical-inference-project).
